...
export default function SignupPage() {
  ...
  const handleOAuthClick = (provider: string) => {
    setError(`${provider} OAuth is ready for credentials. Configure client IDs in Render environment.`);
  };
  ...
  return (
    <div className="min-h-screen bg-canvas flex flex-col justify-center items-center p-4 selection:bg-subtle selection:text-heading">
      ...
      {/* Solid Apple-Crafted Signup Card */}
      <div className="w-full max-w-md bg-surface border border-subtle rounded-2xl p-8 sm:p-10 shadow-sm">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h1 className="text-xl font-semibold text-heading tracking-tight">
            Create your Aegis workspace
          </h1>
          <p className="text-xs text-muted mt-1 max-w-xs">
            Deploy continuous secret leak interception across your repositories
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button
            ...
            className="flex items-center justify-center space-x-2 bg-surface hover:bg-canvas border border-subtle hover:border-interactive rounded-xl py-2.5 px-3 text-xs font-medium text-heading transition-all"
          >
            <svg className="w-4 h-4 text-heading" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-[11px]">
            <span className="bg-surface px-3 text-muted">or register with email</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2.5 bg-primary hover:bg-heading text-surface text-xs font-semibold rounded-xl transition-colors disabled:opacity-50 mt-2 cursor-pointer"
          >
        ...
        <div className="text-center mt-6 text-xs text-muted">
          ...
          <Link
            href="/login"
            className="text-primary hover:text-heading font-medium underline underline-offset-4 transition-colors"
          >
            Sign in
          </Link>
        </div>
